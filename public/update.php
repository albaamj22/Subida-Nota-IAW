<?php
    ...
    $q="select id, nombre, descripcion, categoria, disponible, precio from videojuegos where id=?";
    $stmt=mysqli_stmt_init($conexion);
    mysqli_stmt_prepare($stmt,$q);
    mysqli_stmt_bind_param($stmt,'i',$id);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_bind_result($stmt,$i, $nom, $des, $cat, $dis, $pre);
    ...
    $estaDisponible=($dis=="SI")? "checked" : "";
    $noEstaDisponible=($dis=="NO")? "checked" : "";
    ...
    if(isset($_POST['nombre'])){
        ...
        $q="update videojuegos set nombre=?, descripcion=?, categoria=?, disponible=?, precio=? where id=?";
        $stmt=mysqli_stmt_init($conexion);
        mysqli_stmt_prepare($stmt,$q);
    ...
    <div class="max-w-2xl mx-auto bg-white shadow-xl rounded-xl p-8 space-y-6">
        ...
        <form action="update.php?id=<?= $id ?>" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" value="<?= $nom ?>" name="nombre" required
            ...
            <div>
                ...
                <textarea name="descripcion" rows="4"
                    class="w-full border border-purple-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500 focus:outline-none"><?= $des ?></textarea>
            ...
            <div>
                ...
                <select name="categoria"
                    ...
                    <?php foreach($clCategorias as $categoria=>$nose):?>
                    <option <?= $cat==$categoria ? 'selected' : '' ?>><?= $categoria ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <div>
                ...
                <div class="flex items-center gap-6">
                    <label class="flex items-center gap-2">
                        <input type="radio" name="disponible" value="SI" class="accent-purple-600" <?= $estaDisponible ?>/>
                        SI
                    </label>

                    <label class="flex items-center gap-2">
                        <input type="radio" name="disponible" value="NO" class="accent-purple-600" <?= $noEstaDisponible ?>/>
                        NO
                    </label>
                </div>
            ...
            <div>
                ...
                <input type="number" value="<?= $pre ?>" name="precio" step="0.01" min="0"
                    class="w-full border border-purple-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500 focus:outline-none" />
